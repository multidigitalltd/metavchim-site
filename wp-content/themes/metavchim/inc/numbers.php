 ...
 *
 * הערכים נערכים בלוח הבקרה. כל עוד לא הוזנו מספרים, מוצגת הרצועה
 * המקורית של התבנית — כדי שלא יופיעו באתר נתונים שלא אושרו.
 *
 * @package Metavchim
 */
...
/**
 * רצועת ברירת המחדל, כשעדיין לא הוזנו מספרים.
 *
 * @return array<int,array{value:string,label:string}>
 */
function mv_numbers_default() {
	return array(
		array(
			'value' => 'אפס',
			'label' => 'הקלדות ידניות אחרי שיחה נכנסת',
		),
		array(
			'value' => 'בזמן אמת',
			'label' => 'הצלבה של כל נכס מול כל הקונים',
		),
		array(
			'value' => 'דו־צדדי',
			'label' => 'אישור לפני חשיפת פרטי לקוח לשותף',
		),
		array(
			'value' => 'עברית מלאה',
			'label' => 'ממשק RTL, תמלול עברית, נגישות',
		),
	);
}
 ...
 */
function mv_numbers() {
	$saved = mv_numbers_saved();
	return $saved ? $saved : mv_numbers_default();
}
...
function mv_render_numbers_page() {
	...
	<div class="wrap">
		...
		<p>
			...
			<?php if ( ! $rows ) : ?>
				<strong>כרגע מוצגת רצועת ברירת המחדל של התבנית</strong> — ברגע שתמלאו כאן מספרים היא תוחלף בהם.
			<?php endif; ?>
		</p>
